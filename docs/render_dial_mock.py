...
def render(minutes_left, total, layer, batt, profile, mods, usb=False, armed=False):
    ...
    # Japanese layer names come from the MPLUS1_JP_20 fallback, nominally the
    # same size as the Latin face. They still render larger: an M PLUS glyph box
    # is 18px tall against a 14px Latin cap height. That is the point -- kanji
    # carry more detail and need the body to read at a glance -- and it is why
    # both scripts can share one font size here.
